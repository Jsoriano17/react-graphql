...
//@ts-ignore
import { Select, TextInput, DatePicker } from "react-native-web-ui-components";
  ...
  return (
    <View>
      <TextInput
        ...
        editable={!disabled}
        onChangeText={(text: any) => props.state.setItemValue(props.name, text)}
      />
    </View>
  );
};
  ...
  function SelectViaRelationship(props) {
    ...
    console.log(
      ...
      queryState.items?.map?.(
        (itm) => (itm as any)?.[relationshipColumnName as any]
      )
    );
    ...
    const options = queryState.items?.map?.(
      (itm: any) => itm?.[relationshipColumnName as any]
    );
